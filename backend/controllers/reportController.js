...
export const exportReport = async (req, res, next) => {
  try {
    ...
    if (type === 'customers') {
      ...
      if (format === 'csv') {
        csvContent = 'Name,Phone,Alternate Phone,Email,Address Line,Aadhaar,GST,Plan Name,Allowed Devices,Devices Used,Payment Status,Status\n';
        for (const c of customers) {
          ...
          const address = `"${c.addressLine || ''} ${distStr} ${stateStr}"`.replace(/\n/g, ' ');
          csvContent += `"${c.name}","${c.phone}","${c.alternatePhone || ''}","${c.email || ''}",${address},"${c.aadhaarNumber || ''}","${c.gstNumber || ''}","${c.planName || ''}",${c.devicesAllowed || 0},${c.devicesUsed || 0},"${c.paymentStatus || ''}","${c.subscriptionStatus || ''}"\n`;
        }
      } else {
        data = customers;
      }
